...
static void* demo_mount(struct fuse_conn_info * conn_info){
    ...
    /* 填充super信息 */
    ddriver_ioctl(super.driver_fd, IOC_REQ_DEVICE_IO_SZ, (void *)&super.sz_io);
    ddriver_ioctl(super.driver_fd, IOC_REQ_DEVICE_SIZE, (void *)&super.sz_disk);
    super.sz_blks = 2 * super.sz_io;    // logic block is 2 times of io size 
...
static int demo_readdir(const char* path, void* buf, fuse_fill_dir_t filler, off_t offset, struct fuse_file_info* fi)
{
    ...
    ddriver_seek(super.driver_fd, 500 * super.sz_blks, SEEK_SET);

    /* TODO: 调用ddriver_read读出一个磁盘块到内存，512B */
    char read_buf[super.sz_blks];
    ddriver_read(super.driver_fd, read_buf, super.sz_io);
    ddriver_read(super.driver_fd, read_buf + super.sz_io, super.sz_io);

    /* TODO: 使用memcpy拷贝上述512B的前sizeof(demo_dentry)字节构建一个demo_dentry结构 */
    struct demo_dentry dentry;
    memcpy(&dentry, read_buf, sizeof(struct demo_dentry));


    /* TODO: 填充filename */
    strncpy(filename, dentry.fname, sizeof(filename));
